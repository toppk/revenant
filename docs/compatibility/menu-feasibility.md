...
| logging | Tee PTY output to `logFile`, honour `logInhibit` | None (before `vt_write`) | One coherent PTY tee with `-/+l` and `-lf`; tracked as TODO P1 | ROADMAP |
...
| clearsavedlines | Reset + drop scrollback | `ghostty_terminal_reset()` performs RIS and clears scrollback | Add the action and Meta+Button-2 binding; test reset state and `DATA_SCROLLBACK_ROWS` against xterm | READY |
...
| font-linedrawing | Internal box glyphs | None | **Procedural Glyphs** label; toggles `forceBoxChars` for every supported procedural range | DONE |
